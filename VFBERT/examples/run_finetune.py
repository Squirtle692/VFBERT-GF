            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if (
                        ...
                        print('evaluate11111111111111111111111111')
...
def predict(args, model, tokenizer, prefix=""):
    ...
    for pred_task, pred_output_dir in zip(pred_task_names, pred_outputs_dirs):
        ...
        print('args.per_gpu_pred_batch_size:', args.per_gpu_pred_batch_size)
        ...
        for batch in tqdm(pred_dataloader, desc="Predicting"):
            ...
            model.classifier = nn.Linear(768, 768).to(device)
            ...
            with torch.no_grad():
                ...
                # outputs, pool_output = model(**inputs)
                # _, logits = outputs[:2]
                pool_output = model(**inputs)
                print('pool_output2222:', pool_output.shape)
                pred = pool_output.cpu().detach().numpy().tolist()
                for i in range(len(pred)):
                    all_pred.append(pred[i])
                print('saved')
    for i in range(len(all_pred)):
        print(i, ':', np.sum(all_pred[i]))
        features[i] = all_pred[i]
    print('len features:', len(features))
    print('features[0]:', len(features[0]))
    with open('bert_ori.pkl', 'wb') as f:
...
def main():
    ...
    if not args.do_visualize and not args.do_ensemble_pred:
        ...
        print('config.split111111111111111111:', config.split)
        config.rnn = args.rnn
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        prefix = ''
        model = model_class.from_pretrained(checkpoint)
        print('args.device2222222222222:', args.device)
        model.to(args.device)
        #model.to('cpu')
